...
EMBEDDINGS_CSV_PATH = f"{EMBEDDINGS_DIR}/lds-scriptures-chunked-embeddings.csv"
...
def create_embeddings():
    if not os.path.exists(EMBEDDINGS_CSV_PATH):
        os.makedirs(os.path.dirname(EMBEDDINGS_DIR), exist_ok=True)
    ...
    def add_data_in_batches(df, batch_size):
        for start in range(0, len(df), batch_size):
            ...
            collection.add(
                ...
                embeddings=[
                    row["embedding"].tolist() for index, row in batch_df.iterrows()
                ],
...
def query_chromadb(text_query, max_results=30):
    ...
    if len(quoted_phrases) == 0:
        ...
        results = collection.query(
            ...
            n_results=max_results,
            where_document={
                "$and": [{"$contains": phrase} for phrase in quoted_phrases]
            },
        )
    return results
...
def search():
    ...
    doctrine_and_covenants_enabled = request.args.get("dc", "true").lower() in [
        "true",
        "1",
    ]
    ...
    if threshold:
        ...
        metadatas = [
            metadata for metadata in metadatas if metadata["distance"] <= threshold
        ]
    ...
    if not book_of_mormon_enabled:
        metadatas = [
            ...
            if metadata["volume_title"] != "Book of Mormon"
        ]
    if not doctrine_and_covenants_enabled:
        metadatas = [
            metadata
            for metadata in metadatas
            if metadata["volume_title"]
            not in ["Doctrine and Covenants", "Pearl of Great Price"]
        ]
    if not old_testament_enabled:
        metadatas = [
            metadata
            for metadata in metadatas
            if metadata["volume_title"] != "Old Testament"
        ]
    if not new_testament_enabled:
        metadatas = [
            metadata
            for metadata in metadatas
            if metadata["volume_title"] != "New Testament"
        ]
...
if __name__ == "__main__":
    ...
    app.logger.info("Scriptures downloaded and unzipped.")
    create_embeddings()
    app.logger.info("Embeddings created.")
